...
export default function MemberMembershipPage() {
  ...
  const remainingDays = current
    ? Math.max(
        0,
        Math.ceil((new Date(current.endDate).getTime() - Date.now()) / (24 * 60 * 60 * 1000)),
      )
    : 0;
  ...
  const renewMutation = useMutation({
    mutationFn: () =>
      membershipApi.renew({
        packageId: selectedPackageId || current?.packageId || '',
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-10 text-center">
            <CreditCard className="mx-auto size-12 text-muted" />
            ...
            <div className="flex items-start gap-3 rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 text-amber-300">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs">
                  Còn {remainingDays} ngày ({formatDate(current.endDate)}). Gia hạn ngay để tiếp tục
                  tập luyện không gián đoạn.
                </p>
              </div>
            </div>
          )}
            ...
            <CardContent>
              {history.length === 0 ? (
                <p className="py-6 text-center text-sm text-muted">
                  Chưa có gói tập nào trong lịch sử.
                </p>
              ) : (
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                {packages.map((p) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-sm font-bold text-neon">{formatCurrency(p.price)}</p>
          ...
          <div>
            ...
            <div className="space-y-2">
              {PAYMENT_METHODS.map((m) => (
                ...
                >
                  <CheckCircle2
                    className={`size-4 shrink-0 ${selectedMethod === m.value ? 'text-neon' : 'text-muted'}`}
                  />
